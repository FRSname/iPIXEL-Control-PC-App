...
class SpriteFont:
    ...
    def load(self) -> bool:
        ...
        # Calculate tile dimensions - use configured cols value
...
class SpriteFontService:
    ...
    def render_text_line(
        ...
        # Create base with max(1, total_w) to avoid zero-width images
        base = Image.new("RGBA", (max(1, total_w), font.tile_height), bg_color)

        x = 0
        for ch in chars:
            glyph = font.get_glyph(ch)
            if glyph is None:
                glyph = font.get_glyph(ch.upper())
            if glyph is None:
                # Skip unknown characters but advance position
                x += font.tile_width
                continue
            # Use glyph as mask to preserve transparency
            base.paste(glyph, (x, 0), glyph)
            x += font.tile_width

        # Keep as RGBA for further processing
        return base, None
    ...
    def render_text_fixed(
        ...
        Returns:
            Tuple of (PIL Image, error message or None)
        """
        # First render as line (returns RGBA)
        line_img, error = self.render_text_line(text, font_name, bg_color)
        if error:
            return None, error

        # Handle sizing to target dimensions
        if line_img.size != (width, height):
            # If height matches but width is larger, crop to center (don't squeeze)
            if line_img.height == height and line_img.width > width:
                # Center crop - keeps characters at proper size
                left = (line_img.width - width) // 2
                cropped = line_img.crop((left, 0, left + width, height))
                # Already correct size, just return
                return cropped.convert("RGB"), None
            elif line_img.height <= height and line_img.width <= width:
                # Smaller than target - center on canvas without scaling
                canvas = Image.new("RGBA", (width, height), bg_color)
                offset_x = (width - line_img.width) // 2
                offset_y = (height - line_img.height) // 2
                canvas.paste(line_img, (offset_x, offset_y), line_img)
                line_img = canvas
            else:
                # Scale to fit (only when both dimensions need adjustment)
    ...
    def render_text(
        ...
        try:
            if fixed_size:
                ...
                # render_text_line returns RGBA, convert to RGB for consistency
                img, error = self.render_text_line(text, "_temp", bg_color)
                if img:
                    img = img.convert("RGB")
                return img, error
        finally:
